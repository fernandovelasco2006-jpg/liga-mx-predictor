...
with tab_hist_ap:
    ...
    if not SUPABASE_DISPONIBLE:
        ...
        if not historial:
            ...
            with c5:
                st.markdown(f'<div class="metric-box"><div class="metric-val" style="color:#8fbfa0">{stats.get("total_nulas", 0)}</div>'
                            f'<div class="metric-lbl">⚪ Nulas (DNB empatado)</div></div>', unsafe_allow_html=True)
...
with tab_parlays:
    ...
    if not SUPABASE_DISPONIBLE:
        ...
        # Capital inicial con el que se empezó a apostar (fijo, no viene
        # de Supabase — es un dato de referencia del usuario, no algo
        # que cambie por resultado de apuestas). El "saldo actual" es
        # informativo: capital inicial + ganancia/pérdida neta real. El
        # ROI en las métricas de abajo sigue siendo % sobre lo apostado
        # en cada boleto (no sobre este capital), que es la medida
        # correcta de "qué tan bueno es el criterio de apuesta" — mezclar
        # ambas cosas (ROI sobre capital total) daría un número distinto
        # y menos útil para evaluar si vale la pena seguir apostando.
        CAPITAL_INICIAL_MXN = 100.0
        saldo_actual = CAPITAL_INICIAL_MXN + roi["ganancia_neta_total"]
        color_saldo = "#4ade80" if saldo_actual >= CAPITAL_INICIAL_MXN else "#f87171"
        st.markdown(
            f'<div class="metric-box" style="max-width:280px;margin-bottom:0.8rem">'
            f'<div class="metric-val" style="color:{color_saldo}">${saldo_actual:.0f}</div>'
            f'<div class="metric-lbl">Saldo actual (inició con ${CAPITAL_INICIAL_MXN:.0f} MXN)</div></div>',
            unsafe_allow_html=True,
        )
